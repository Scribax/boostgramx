...
// GET /api/services/test - Ruta de prueba
router.get('/test', (req, res) => {
  res.json({
    success: true,
    message: 'Ruta de servicios funcionando',
    memoryDB: !!global.USE_MEMORY_DB,
...
router.get('/', async (req, res) => {
  try {
    ...
    // Usar base de datos en memoria si no hay MongoDB
    if (global.USE_MEMORY_DB) {
      // Construir filtros para memoryDB
      ...
      // Obtener servicios desde memoria
      const services = await memoryDB.findServices(filters, {
        sort: { popularityScore: -1 },
        skip,
        limit: parseInt(limit)
      });
      
      const total = await memoryDB.countServices(filters);
